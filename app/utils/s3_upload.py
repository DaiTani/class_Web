...
def upload_to_s3(file, album_id):  # 移除acl参数
    ...
    try:
        ...
        s3 = boto3.client(
            ...
            config=s3_config,
            verify=os.path.join(os.path.dirname(__file__), 'cacert.pem')  # 添加SSL证书验证路径
        )
...
def delete_from_s3(s3_url):
    ...
    try:
        ...
        logger.warning(f"SSL验证失败，可能文件已手动删除: {str(e)}")
        return True 
    except Exception as e:
...
# 确保S3 URL生成格式正确
def generate_s3_url(bucket_name, region, key):
    return f"https://{bucket_name}.s3.{region}.amazonaws.com/{key}"

# 确保上传时正确保存URL到数据库
def upload_to_s3(file, album_id):
    """上传文件到AWS S3并返回URL"""
    # 生成唯一文件名
    filename = f"album_{album_id}_{uuid.uuid4()}_{os.path.basename(file.filename)}"
    s3_key = f"memories/{filename}"
